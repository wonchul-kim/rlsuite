...
def get_version():
    file = PARENT / 'rlsuite/version.py'
...
version=get_version()
packages=['rlsuite'] + [str(x) for x in Path('rlsuite').rglob('*/') if x.is_dir() and '__' not in str(x)]
...
setup(
    ...
    packages=['rlsuite'] + [str(x) for x in Path('athena').rglob('*/') if x.is_dir() and '__' not in str(x)],
    package_data={
        '': ['*.yaml'],
    },
