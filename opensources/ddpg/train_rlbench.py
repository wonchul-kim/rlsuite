...
def train(num_iterations, agent, env,  evaluate, validate_steps, output, max_episode_length=None, debug=False):
    ...
    while step < num_iterations:
        ...
        print(f'\r step: {step} >> reward: {reward} | terminated: {terminated} | truncated: {truncated}', end='')
...
class FetchObservationOnlyWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
        # observation_space도 원래 observation 부분만으로 수정
        self.observation_space = env.observation_space["observation"]

    def observation(self, observation):
        # obs dict에서 observation 키만 추출
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_episode_length', default=50, type=int, help='')
    ...
    if args.mode == 'train':
        ...
        env = NormalizedEnv(gym.make(args.env, render_mode='human'))
    else:
    ...
    env = FetchObservationOnlyWrapper(env)
